...
#define NAPI_INT64(name, val) \
  int64_t name;               \
  napi_get_value_int64(env, val, &name);
...
static void on_uv_timeout(uv_timer_t *handle)
{
  NUV_CALLBACK_START(handle, "onTimeout")
  NUV_CALLBACK_ARGS(0)
  NUV_CALLBACK_CALL()
  NUV_CALLBACK_END()
}
...
static void on_uv_read(uv_stream_t *handle, ssize_t nread, const uv_buf_t *buf)
{
  ...
  NUV_CALLBACK_ARGS(2)
  if (nread < 0)
    nuv_create_status(env, nread, &argv[0]);
  else
...
NAPI_METHOD(nuv_timer_init)
{
  NAPI_ARGV(2)
  NAPI_BUFFER_CAST(uv_timer_t *, handle, argv[0])
  int NAPI_UV_THROWS(err, uv_timer_init(uv_default_loop(), handle));
  nuv_context_t *context = malloc(sizeof(*context));
  handle->data = context;
  context->env = env;
  napi_create_reference(env, argv[1], 1, &(context->this));
...
NAPI_METHOD(nuv_timer_start)
{
  NAPI_ARGV(3)
  NAPI_BUFFER_CAST(uv_timer_t *, handle, argv[0])
  NAPI_INT64(timeout, argv[1])
  NAPI_INT64(repeat, argv[2])
  int NAPI_UV_THROWS(err, uv_timer_start(handle, on_uv_timeout, timeout, repeat));
  return NULL;
}

NAPI_METHOD(nuv_timer_stop)
{
  NAPI_ARGV(1)
  NAPI_BUFFER_CAST(uv_timer_t *, handle, argv[0])
  int NAPI_UV_THROWS(err, uv_timer_stop(handle));
  return NULL;
}

NAPI_METHOD(nuv_timer_again)
{
  NAPI_ARGV(1)
  NAPI_BUFFER_CAST(uv_timer_t *, handle, argv[0])
  int NAPI_UV_THROWS(err, uv_timer_again(handle));
  return NULL;
}

NAPI_METHOD(nuv_timer_set_repeat)
{
  NAPI_ARGV(2)
  NAPI_BUFFER_CAST(uv_timer_t *, handle, argv[0])
  NAPI_INT64(repeat, argv[1])
  uv_timer_set_repeat(handle, repeat);
  return NULL;
}

NAPI_METHOD(nuv_timer_get_repeat)
{
  NAPI_ARGV(1)
  NAPI_BUFFER_CAST(uv_timer_t *, handle, argv[0])
  napi_value repeat;
  napi_create_int64(env, uv_timer_get_repeat(handle), &repeat);
  return repeat;
}
...
NAPI_INIT()
{
  ...
  // Timer Functions
  NAPI_EXPORT_FUNCTION(nuv_timer_init)
  NAPI_EXPORT_FUNCTION(nuv_timer_start)
  NAPI_EXPORT_FUNCTION(nuv_timer_stop)
  NAPI_EXPORT_FUNCTION(nuv_timer_again)
  NAPI_EXPORT_FUNCTION(nuv_timer_set_repeat)
  NAPI_EXPORT_FUNCTION(nuv_timer_get_repeat)
  // Handle Functions
